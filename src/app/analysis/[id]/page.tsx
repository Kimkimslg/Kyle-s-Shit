...
import { 
  ...
  Calendar, Folder, Clock, User, Loader2
} from 'lucide-react';
  ...
  if (!analysis) {
    return (
      <div className="max-w-4xl mx-auto px-8 py-20 text-center">
        ...
        <p className="text-secondary mt-4">요청하신 분석물을 찾을 수 없거나 아직 서버에 반영되지 않았습니다.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-20 lg:py-32">
      <header className="mb-16">
        <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-secondary mb-6">
          <span className="flex items-center gap-1.5">
            <Calendar className="w-3 h-3" /> {new Date(analysis.created_at).toLocaleDateString()}
          </span>
          <span className="flex items-center gap-1.5">
            <Folder className="w-3 h-3" /> {analysis.industry_id}
          </span>
          <span className="flex items-center gap-1.5">
        ...
        <div className="flex items-center justify-between border-y border-border py-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-secondary uppercase tracking-[0.1em]">Archive Contributor</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <article className="space-y-20">
        <div className="bg-muted p-8 rounded-xl border-l-4 border-primary italic text-lg leading-relaxed text-slate-700">
        ...
        {sections.map((section) => (
          <section key={section.key} className="space-y-6">
            ...
            <div className="analysis-content text-slate-700 leading-loose whitespace-pre-wrap">
            ...
            <ul className="space-y-3">
              {analysis.reference_links.map((ref, i) => (
                <li key={i} className="text-xs text-secondary font-medium flex items-center gap-2">
                  <span className="w-1 h-1 bg-slate-300 rounded-full" /> {ref}
                </li>
              ))}
            </ul>
          </section>
        ...
        {/* Real-time Interaction Section (Supabase Integrated) */}
      ...
      <footer className="mt-32 pt-16 border-t border-border flex flex-col items-center gap-8">
        <Link href="/" className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary hover:text-primary transition-colors flex items-center gap-3">
